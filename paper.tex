...
equal-width confidence bins, so the model is mildly overconfident; the
reliability diagram is reported in the supplementary figures rather than the
main text.
...
The calibration measurement is more informative. The expected calibration error
is 0.0717, with mean confidence exceeding observed accuracy, so the model is
mildly overconfident --- and the direction of its errors is systematic rather
than symmetric: Osteoporosis recall (0.8151) is the lowest in
Table~\ref{tab:per_class_performance} while Osteopenia precision (0.7429) is
also the lowest, which is the signature of a decision boundary displaced toward
the intermediate grade. With a top-2 accuracy of 97.61\%, the ranking is close
to exhausted as a source of further gains; what remains is the mapping from
probabilities to a decision. Per-class threshold adjustment or a
temperature-scaled, cost-sensitive rule fitted on the validation partition is
therefore the cheapest remaining improvement, and it targets exactly the
